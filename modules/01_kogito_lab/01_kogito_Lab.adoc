...
This version of the Kogito Travel Agency app is dependent on a running Kafka cluster. We could spin up a local containerized Kafka cluster, but forthe sake of simplicity, we'll deploy the app directly to OpenShift, using a native image.
...
. Review the `pom.xml` file of the the application. Notice the dependencies to `org.kie.kogito:kogito-events-reactive-messaging-addon`, `io.quarkus:quarkus-kafka-client` and `io.quarkus:quarkus-smallrye-reactive-messaging-kafka` which are required for Kafka integration.
...
. Check the logs of the Kafka Consumer app. Note that several events have been sent by the applications to the Kafka topic. The events contain information about the process instance, the nodes triggered and the state of the process variables: 
+
----
