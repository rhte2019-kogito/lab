...
When you run `mvn compile quarkus:dev`, the Kogito/Quarkus application will launch in development mode. When it receives a HTTP request it will hold the request, and check to see if any application source files (Java code, process definitions, rule definitions, etc.) have been changed. If they have it will transparently compile the changed files, redeploy
the application with the changed files, and then the HTTP request will continue to the redeployed application. Project redeploys are much faster than a traditional app server, so for all but the largest applications this should take well under a second. This greatly speeds up a developer's development cycle.
...
In Kogito, the exposed (RESTful) services are generated based on your domain data. In other words, Kogito adapts to your business domain rather than the other way around. This prevents the leaking of abstractions of the platform into your client applications and services and allows you to stay focused on the business and business domain instead of being concerned with the technology behind it.
...
A Travel request can be started via Swagger UI - using its _Try out_ option. Locate the `/travels` endpoint with `POST` method with following payload
...
Kogito supports runtime persistence for workflows. That allows users to configure key value based storage backed by Infinispan to persist data (including active nodes and process instance variables) to preserve the state across restarts.

Runtime persistence focuses mainly on storing data that are required to resume workflow execution for a particular process instance. Regardless if the process is public or private, as long as it’s not completed it subject for persistence.

*Node instances*

Node instances that are currently active, so called wait states, are persisted when the process instance finishes execution but has not reached an end state (completed or aborted). This means that only information required to resume is persisted. There might be one or more active nodes instances stored at any given point in time.

*Process instance variables*

All data that are included in the process instance are stored together with the process instance itself. These must be marshalled into a bytes format so it can be easily transferred over the wire and persisted into the key value storage. The marshalling and unmarshalling is implemented based on ProtoBuf and requires to have schema and marshallers available to know how to deal with a given type of data.
...
+
----
05:35:11,172 INFO  [org.jboss.as.clustering.infinispan] (async-thread--p6-t1) DGISPN0001: Started travels_store cache from clustered container
05:35:11,203 INFO  [org.jboss.as.clustering.infinispan] (async-thread--p6-t2) DGISPN0001: Started flightBooking_store cache from clustered container
05:35:11,219 INFO  [org.jboss.as.clustering.infinispan] (async-thread--p6-t3) DGISPN0001: Started hotelBooking_store cache from clustered container
----
...
. By default, the Kogito Travel Agency app will try to connect to a Infinispan server on `localhost:11222`. When deploying on OpenShift, we need to configure the application to connect to a remote Infinispan instance. To override the default configuration properties we can use a _ConfigMap_ containing an `application.properties` file which will be mounted inside the application pod.
...
. The `openshift` folder of the `02-kogito-travel-agency` project contains an OpenShift template file for a binary deployment of the Kogito Travel Agency app. It describes a BuildConfig, ImageStream, DeploymentConfig, Service and Route for the application. In this lab, we will deploy the application itself using a binary build from the lab workstation. +
...
----
+
`kogito-travel-agency-1.0-SNAPSHOT-runner.jar` is an executable JAR file. It is however not an _uber-jar_ as the dependencies are copied into the target/lib directory.
...
NOTE: On the hosted cluster for this lab a S2I build takes considerably longer than a native build on the workstation. The S2I build can take up to 20 minutes to complete. 
...
==== Native binary and image build

If you prefer to build the application using S2I on the OpenShift cluster, skip this section and proceed to the next section.
...
If you have built and deployed the native binary and image on the workstation as described in the previous section, you can skip this section of the lab.

. This S2I build requires more than 6Gi of memory. The `kogito2` project has a LimitRange defined which limits the maximum memory for a container to 6Gi. To allow for more memory, the LimitRange needs to be patched:
.. Log in into the cluster as user `admin`. Use the same password as `user1`.
+
----
$ oc login -u admin
----
.. Patch the LimitRange:
+
----
$ oc patch limitrange kogito2-core-resource-limits --type='json' -p='[{"op": "replace", "path": "/spec/limits/0/max/memory", "value": "8Gi" }]' -n kogito2
----
+
.Output
----
limitrange/kogito2-core-resource-limits patched
----
.. Log back in as `user1`.
+
----
...
----
. Review the `openshift/kogito-travel-agency-s2i.yml` template for the S2i build.
* This template define ImageStreams, BuildConfigs, DeplopymentConfigs, Services and Routes for a S2I build - including a chained build - and deployment of the Kogito Travel Agency application.
. Deploy the template for the S2I build:
+
----
$ oc process -f openshift/kogito-travel-agency-s2i.yml | oc create -f - -n kogito2
----
+
.Output
----
...
imagestream.image.openshift.io/kogito-quarkus-ubi8-s2i created
imagestream.image.openshift.io/kogito-travel-agency-s2i created
buildconfig.build.openshift.io/kogito-travel-agency-s2i-runtime created
imagestream.image.openshift.io/kogito-quarkus-ubi8 created
imagestream.image.openshift.io/kogito-travel-agency-s2i-runtime created
service/kogito-travel-agency-s2i created
route.route.openshift.io/kogito-travel-agency-s2i created
deploymentconfig.apps.openshift.io/kogito-travel-agency-s2i created
----

. Using `oc` client, follow the logs of the builder pod. The build will take up to 10 minutes to complete.
. Once the S2I native build is finished, the chained build starts automatically.
. Once the chained build is finished, the application is deployed.
...
. Build a Linux 64-bit native executable of the Kogito Visas application:
+
----
$ mvn clean package -DskipTests=true -Pnative
----
...
. Review the `openshift/kogito-visas-s2i.yml` template for the S2i build.
* This template define ImageStreams, BuildConfigs, DeplopymentConfigs, Services and Routes for a S2I build - including a chained build - and deployment of the Kogito Visas application.
. Deploy the template for the S2I build:
+
----
$ oc process -f openshift/kogito-visas-s2i.yml | oc create -f - -n kogito2
----
+
.Output
----
buildconfig.build.openshift.io/kogito-visas-s2i created
imagestream.image.openshift.io/kogito-visas-s2i created
buildconfig.build.openshift.io/kogito-visas-s2i-runtime created
imagestream.image.openshift.io/kogito-visas-s2i-runtime created
service/kogito-visas-s2i created
route.route.openshift.io/kogito-visas-s2i created
deploymentconfig.apps.openshift.io/kogito-visas-s2i created
Error from server (AlreadyExists): imagestreams.image.openshift.io "kogito-quarkus-ubi8-s2i" already exists
Error from server (AlreadyExists): imagestreams.image.openshift.io "kogito-quarkus-ubi8" already exists
----
* If you built the Kogito Travel Agency Application with S2I, you will see errors that the `kogito-quarkus-ubi8-s2i` and `kogito-quarkus-ubi8` imagestreams already exist. These errors can be ignored.

. Using `oc` client, follow the logs of the builder pod. The build will take up to 10 minutes to complete.
. Once the S2I native build is finished, the chained build starts automatically.
. Once the chained build is finished, the Kogito Visas application is deployed.
...
.Output
----
configmap/data-index-service created
----
...
.Output
----
configmap/data-index-service-proto created
----
...
+
----
$ oc label service kogito-visas-native monitoring=prometheus -n kogito2
$ oc label service kogito-travel-agency-native monitoring=prometheus -n kogito2
----
+
If you used S2I to build and deploy the Kogito Travel Agency and Kogito Visas, the service names are `kogito-travel-agency-native-s2i` and `kogito-visas-s2i`.
...
+
Note that the list will not show metric types for which data has not been generated yet.
