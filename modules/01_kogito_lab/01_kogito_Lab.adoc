...
. Start a local instance of Infinispan on the lab workstation using Docker. +
Open a new terminal, ssh into the workstation as user `lab-user`, and run the following commands: 
+
----
$ sudo systemctl start docker
$ sudo docker run -it -p 11222:11222 jboss/infinispan-server:10.0.0.Beta3
----
+
...
+
----
$ oc new-project kogito-travel-agency
----
...
+
----
$ oc process -f openshift/infinispan-server.yml | oc create -f - -n kogito-travel-agency
----
...
+
----
quarkus.infinispan-client.server-list=infinispan-server.kogito-travel-agency.svc:11222
quarkus.swagger-ui.always-include=true
----
+
By default, Swagger UI is only available when Quarkus is started in dev or test mode. The `quarkus.swagger-ui.always-include=true` entry will make the Swagger UI available in a production deployment.
.. Create a ConfigMap from the `application.properties` file:
+
----
$ oc create configmap kogito-travel-agency --from-file=/tmp/application.properties -n kogito-travel-agency 
----
...
+
----
$ oc process -f openshift/kogito-travel-agency.yml | oc create -f - -n kogito-travel-agency
----
+
.Output
----
...
----
+
The project unit tests require a running local Infinispan instance, so can skip them.
...
+
----
$ oc start-build kogito-travel-agency --from-dir=/tmp/kogito-travel-agency -n kogito-travel-agency
----
+
----
...
+
----
$ oc logs kogito-travel-agency-1-build
----
+
----
...
. Test the Kogito Travel Agency application using the application UI or the Swagger
